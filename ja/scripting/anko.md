...
デフォルトでは、ankoスクリプトは、httpおよびnetライブラリ、sftp、sshなどのネットワークユーティリティを使用できます。 Gravwellユーザーにネットワークアクセスを許可したくない場合があります。`Disable-Network-Script-Functions=true' in `でオプション`/opt/gravwell/etc/gravwell.conf`を設定すると、これが無効になります。
...
重要：Parse関数は明示的に値を返さなければなりません。 nilを返すと、解析が成功したことを示します。 それ以外のものを返すと、エラーを示します。 エラーが発生した場合は、問題を説明する文字列を返すことをお勧めします。
...
* `producesEnum（val）`スクリプトがその名前の列挙値の生成を計画していることをパイプラインに通知します。 Parse（）関数で呼び出す必要があります。
* `consumesEnum（val）`スクリプトがその名前の列挙値の消費を計画していることをパイプラインに通知します。 Parse（）関数で呼び出す必要があります。
