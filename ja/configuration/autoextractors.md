...
CSVまたは「カンマ区切り値」は、比較的効率的なテキスト転送およびストレージシステムです。 ただし、CSVデータは自己記述的ではありません。つまり、CSVデータが大量にある場合、実際にどの列であるかを判断することは困難です。 自動抽出機能を使用して列名を事前定義し、CSVデータの操作を劇的に簡単にすることができます。
...
```
特定のフィールドの抽出に興味がない場合は、名前を空白のままにしておくことができます。 たとえば、上記の例でGUIDを抽出する必要がない場合、paramsフィールドを  `params="ts, name, id,, src, srcport, dst, dstport, data, country, city, hash"`に変更できます。
