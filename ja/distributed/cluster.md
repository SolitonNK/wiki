...
[クイックスタートドキュメント](#!quickstart/quickstart.md#Debian_repository)の説明に従って、リポジトリをセットアップし、Gravwellパッケージをインストールします。 構成ファイルを独自のもので置き換えるため、プロンプトが表示されたときにインストーラーが秘密トークンを自動生成できるようにすることができます。
